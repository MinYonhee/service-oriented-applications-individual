...
router.get('/', async (req, res) => {
    try {
        const result = await db.query('SELECT * FROM curriculos ORDER BY id');
...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const curriculoResult = await db.query('SELECT * FROM curriculos WHERE id = $1', [id]);
        ...
        const experienciasResult = await db.query('SELECT * FROM experiencias WHERE curriculo_id = $1 ORDER BY data_inicio DESC', [id]);
        curriculo.experiencias = experienciasResult.rows;

        const formacoesResult = await db.query('SELECT * FROM formacoes WHERE curriculo_id = $1 ORDER BY data_conclusao DESC', [id]);
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await db.query('DELETE FROM curriculos WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
...
router.post('/:curriculo_id/experiencias', async (req, res) => {
    ...
    try {
        const curriculoCheck = await db.query('SELECT id FROM curriculos WHERE id = $1', [curriculo_id]);
